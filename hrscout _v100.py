        ...
        for row in rows:
            ...
            if len(cells) >= 4:
                try:
                    ...
                    fd_odds = cells[1].get_text(strip=True).replace("+", "")
                    mgm_odds = cells[2].get_text(strip=True).replace("+", "")
                    if name and fd_odds.lstrip("-").isdigit():
                        hr_odds_cache[name.lower()] = {
                            "fd": int(fd_odds),
                            "mgm": int(mgm_odds) if mgm_odds.lstrip("-").isdigit() else int(fd_odds),
                        }
                except:
                    continue
    except:

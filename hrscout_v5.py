...
CHAT_ID = "919904024"
SCAN_INTERVAL = 300
PARLAY_DISPLAY_MIN = 55  # Used only for parlay — not game display
HOT_MIN = 3
INTERCHANGE_GAP = 3
PARLAY_MIN_CONF = 62
...
# ============================================================
#  HR ODDS CACHE — FanDuel + BetMGM via scraper
# ============================================================
hr_odds_cache = {}
...
async def load_hr_odds(session):
    ...
    for url in urls:
        try:
            ...
            pattern1 = r">([A-Z][a-z]+ [A-Z][a-zA-Z\.\-]+)<[^>]*>[^<]*<[^>]*>\s*([+\-]\d+)\s*<[^>]*>[^<]*<[^>]*>\s*([+\-]\d+)"
            # Pattern 2: simpler
            pattern2 = r"([A-Z][a-z]+ [A-Z][a-zA-Z]+).*?([+]\d{3,4}).*?([+]\d{3,4})"
            for pattern in [pattern1, pattern2]:
                matches = re.findall(pattern, html)
                for match in matches:
                    try:
                        name = match[0].strip()
                        fd = int(match[1].replace("+", ""))
                        mgm = int(match[2].replace("+", ""))
                        if name and len(name) > 4 and abs(fd) > 100:
...
# ============================================================
#  SCORING — Fix #5 #11: Recalibrated for wider spread 75%+
# ============================================================
def score_barrel(b):
    if b >= 20: return 100
    if b >= 15: return 90
...
# Fix #6: Situation weighted over reputation
...
def resolve_tiebreaker(p1, p2):
    ...
    # EV check — only swap if EV gap is meaningful (5%+)
...
async def get_batter_stats(session, player_id):
    ...
    try:
        ...
        # Better estimates using HR rate + ISO + SLG combined
        hr_rate = hr / max(ab, 1)
        iso = round((slg - avg) * 1000)
        # Barrel: driven by HR rate and ISO together
        barrel = min(25, max(3, (hr_rate * 180) + (iso - 100) * 0.04))
        # EV: ISO and SLG are both strong predictors
        ev = min(98, max(83, 87 + (iso - 130) * 0.04 + (slg - 0.350) * 8))
        # Hard hit: correlated with EV
        hh = min(65, max(22, 32 + (iso - 130) * 0.06 + (slg - 0.350) * 30))
        # Flyball: HR hitters pull the ball in the air more
        fb = min(55, max(22, 30 + hr_rate * 200 + (iso - 130) * 0.03))
        # Pull: power hitters pull more
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for game in games:
            ...
            if not home_lineup and not away_lineup:
                # Lineups not confirmed yet — skip this game
                # Bot will catch it on next 5-minute scan
                continue
            ...
            def make_lines(ranked, opp_p_name):
                lines = []
                for i, p in enumerate(ranked):
                    # Show all confirmed lineup players in game rankings
                    # 55% threshold applies to PARLAY only — not display
                    badge = "🔥" if p["conf"] >= 78 else "✅" if p["conf"] >= 62 else "▪️"
                    # Fix #2: Hot flag only 3+ HR last 7 days
                    hot = " 🌶️" if p["hr7"] >= HOT_MIN else ""
                    line = f"{len(lines)+1}. {p['name']} ({p['hand']}) — {p['conf']}% {badge}{hot}"
                    # Fix #4: Tiebreaker only on #1 — one clean line
                    if i == 0 and len(ranked) > 1:
                        next_p = ranked[1]
                        if next_p["conf"] >= 0:  # Always check tiebreaker regardless of score
                            _, _, decision = resolve_tiebreaker(p, next_p)
                            if decision == "INTERCHANGEABLE":
                                line += f"\n   🔄 INTERCHANGE: {next_p['name']} ({next_p['conf']}%)"
                            elif decision in ["PLATOON_SWAP", "PLATOON_WIN"]:
                                line += f"\n   ⚔️ PLATOON EDGE: {p['platoon_hr9']} vs {next_p['platoon_hr9']} HR/9"
                            elif decision in ["EV_SWAP", "EV_WIN"]:
                                line += f"\n   💰 EV EDGE: {p['ev_score']}% vs {next_p['ev_score']}%"
                    lines.append(line)
                return lines

            away_lines = make_lines(away_ranked, home_p_name)
            home_lines = make_lines(home_ranked, away_p_name)

            # Determine game top pick
            all_cands = away_ranked + home_ranked
            ...
            if all_cands:
                ...
                # Also add interchangeable partner to pool
                if len(all_cands) > 1:
...
• Barrel:{away_p['barrel_against']:.1f}% HH:{away_p['hh_against']:.1f}% L3:{away_p['last3_era']} {pitcher_trend(away_p['era'],away_p['last3_era'])}
{splits_line(away_p)}
...
• Barrel:{home_p['barrel_against']:.1f}% HH:{home_p['hh_against']:.1f}% L3:{home_p['last3_era']} {pitcher_trend(home_p['era'],home_p['last3_era'])}
{splits_line(home_p)}
━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━""".strip()
    ...
    if not parlay_sent and all_top_picks and first_game_time:
        ...
        if 10 <= mins_to_first <= 35:
            should_send = True
        elif now_et_hour >= 12 and mins_to_first <= 120:
            should_send = True

        if should_send:
            # Build pool — deduplicate, require 62%+ for parlay
            seen = set()
            pool = []
            ...
            # Also include lower ranked players if they meet 2 of 3 criteria
            # This catches the Benintendi/Contreras situation
            for p in all_top_picks:
                if p["name"] not in seen and p.get("conf", 0) >= 55:
                    ...
                    # Lowered to 1 — avoids missing legitimate plays
                    # due to incomplete recent form API data
                    if criteria >= 1:
                        seen.add(p["name"])
                        pool.append(p)
            ...
            # STANDARD SORT: Data first, EV as tiebreaker when close
            # When two players within 3% — better EV wins
            def std_sort(p):
                conf = p.get("conf", 0)
                ev = p.get("ev_score") or 0
                # EV only breaks ties — not the primary factor
                ev_bonus = ev * 0.1 if ev > 0 else 0
                return conf + ev_bonus

            # EV SORT: Pure value — odds and EV first
            def ev_sort(p):
                ev = p.get("ev_score") or 0
            ...
            # EV 3-leg uses different players than standard when possible
            ...
            if len(game_times) >= 2:
                ...
                spread_hrs = (latest.replace(tzinfo=datetime.timezone.utc) - earliest.replace(tzinfo=datetime.timezone.utc)).total_seconds() / 3600
                is_full_slate = spread_hrs >= 3
            ...
            def leg(p, i):
                ...
                interchange = " ↔️" if p.get("is_interchange") else ""
                gt = p.get("game_dt")
                time_str = ""
                if gt:
                    try:
                        et_hour = (gt.hour - 4) % 24
                        et_min = gt.minute
                        period = "AM" if et_hour < 12 else "PM"
                        h = et_hour % 12 or 12
                        time_str = f" | {h}:{et_min:02d} {period} ET"
                    except:
                        pass
                return (
                    f"{i}. {p['name']} ({p.get('hand','')}) — {p['conf']}%{hot}{interchange}\n"
            ...
            if len(std_pool) >= 2:
                ...
                msg += "\n\n2️⃣ 2-LEG\n"
                msg += "\n".join(leg(std_pool[i], i+1) for i in range(min(2, len(std_pool))))

                if len(std_pool) >= 3:
                    msg += "\n\n3️⃣ 3-LEG\n"
                    msg += "\n".join(leg(std_pool[i], i+1) for i in range(min(3, len(std_pool))))

                if len(std_pool) >= 4:
                    msg += "\n\n4️⃣ 4-LEG\n"
                    msg += "\n".join(leg(std_pool[i], i+1) for i in range(min(4, len(std_pool))))
...
async def main():
    ...
    await bot.send_message(chat_id=CHAT_ID,
        text="✅ HR SCOUT V5 — FINAL BUILD\n\n"
             "📊 Scoring: Guy 1 + Guy 2 + Situation\n"
             "🔱 Full pitching breakdown + real splits\n"
             "⚔️ Platoon tiebreaker on #1 pick only\n"
             "↔️ Interchangeable players compete via EV\n"
             "🌶️ Hot flag = 3+ HR last 7 days\n"
             "📋 Full lineup shown — no display cutoff\n"
             "🚫 Overnight/Japan games filtered\n"
             "🎯 4 parlays — 15-30 mins before first pitch\n"
             "🌅 Full slate or same window detection\n"
             "💰 FD + BetMGM odds on every leg\n\n"

...
def generate_explanation(batter_name, b, p, park, spot, confirmed):
    ...
    if b["barrel"] >= 15:
        reasons.append(f"elite {b['barrel']}% barrel rate (Guy 1+2: 15%+ is very impressive)")
    elif b["barrel"] >= 10:
        reasons.append(f"{b['barrel']}% barrel rate meets the 10% threshold (Guy 1+2)")
    else:
        reasons.append(f"{b['barrel']}% barrel rate — below ideal but other factors compensate")

    if b["ev"] >= 93:
        reasons.append(f"{b['ev']}mph exit velocity — multiple 100mph+ contact events likely (Guy 1+2)")
    elif b["ev"] >= 90:
        reasons.append(f"{b['ev']}mph exit velocity clears 90mph minimum (Guy 2)")
    else:
        reasons.append(f"{b['ev']}mph exit velocity — below ideal threshold")

    if b["pull"] >= 40:
        reasons.append(f"{b['pull']}% pull rate above 40% — 66% of all HRs are pulled (Guy 2)")
    elif b["pull"] >= 33:
        reasons.append(f"{b['pull']}% pull rate meets 33% minimum (Guy 2)")

    if b["iso"] >= 250:
        reasons.append(f"ISO of {b['iso']} is elite raw power (Guy 1: 250+ threshold)")
    elif b["iso"] >= 140:
        reasons.append(f"ISO of {b['iso']} above MLB average of 140 (Guy 1)")

    if p["hr9"] >= 1.5:
        reasons.append(f"pitcher is highly vulnerable — {p['hr9']} HR/9 allowed (Guy 1: 🚨)")
    elif p["hr9"] >= 1.2:
        reasons.append(f"pitcher allows {p['hr9']} HR/9 — above average vulnerability (Guy 1)")

    trend_diff = p.get("last3_era", p["era"]) - p["era"]
    if trend_diff >= 1.0:
        reasons.append(f"pitcher trending worse — last 3 ERA {p.get('last3_era', p['era'])} vs season {p['era']} (my addition)")

    park_f = PARK_FACTORS.get(park, 100)
    if park_f >= 108:
        reasons.append(f"{park} is hitter-friendly (factor {park_f}) — my addition")
    elif park_f <= 94:
        reasons.append(f"{park} is pitcher-friendly (factor {park_f}) — slight negative")

    if spot in [3, 4]:
        reasons.append(f"batting #{spot} in cleanup — maximum PA and RBI opportunities (my addition)")
    elif spot in [1, 2]:
        reasons.append(f"batting #{spot} — high PA volume (my addition)")

    bh = b.get("hand", "R")
    ph = p.get("hand", "R")
    if bh != ph and bh != "S":
        reasons.append(f"platoon advantage — {bh} hitter vs {ph} pitcher (my addition)")

    if not confirmed:
        reasons.append("lineup projected — confidence adjusts when confirmed")
    ...
    explanation += "."
    if len(reasons) > 3:
        explanation += f" Additional: {'; '.join(reasons[3:])}."
    return explanation
...
async def get_batter_stats(session, player_id):
    ...
    try:
        ...
        if splits:
            ...
            # More generous estimates so filters work correctly
...
async def run_scan(bot, phase="UPDATE", prev_scores=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for game in games:
            ...
            for batter, spot, pitcher, is_confirmed in batters_raw:
                ...
                # Minimum HR production filter — must have at least 5 HRs this season
                if b_stats.get("hr", 0) < 5:
                    total_filtered += 1
                    continue
    ...
    label = "⏰ EARLY LOOK" if phase == "EARLY" else "🔒 FINAL LOCK" if phase == "LOCK" else "🔄 LIVE UPDATE"

    if not alerts:
        if phase in ["EARLY", "LOCK"]:
            await bot.send_message(chat_id=CHAT_ID,
                text=f"🤖 HR SCOUT — {label}\n📅 {today}\n👥 Scanned: {total_scanned} players | Filtered low HR: {total_filtered}\n📭 No plays above {threshold}% threshold.\nNext scan in 5 mins.")
        return {**prev_scores, **new_scores}

    await bot.send_message(chat_id=CHAT_ID,
        text=f"🤖 HR SCOUT — {label}\n📅 {today}\n👥 Scanned: {total_scanned} players\n🎯 Threshold: {threshold}%+\n✅ {len(alerts)} play(s) found:")
...
• Barrel %: {a['barrel']}% {'🔥' if a['barrel']>=15 else '✅' if a['barrel']>=10 else '⚠️'}
• Exit Velocity: {a['ev']} mph {'🔥' if a['ev']>=93 else '✅' if a['ev']>=90 else '⚠️'}
• Pull %: {a['pull']}% {'🔥' if a['pull']>=40 else '✅' if a['pull']>=33 else '⚠️'}
• ISO Power: {a['iso']} {'🔥' if a['iso']>=250 else '✅' if a['iso']>=140 else '⚠️'}
• Hard-Hit %: {a['hh']}%

⚾ PITCHER
• HR/9: {a['p_hr9']} {'🚨' if a['p_hr9']>=1.5 else '⚠️' if a['p_hr9']>=1.2 else ''}
• Barrel Allowed: {a['p_barrel']}% {'🚨' if a['p_barrel']>=11 else ''}
• ERA: {a['p_era']} | Last 3: {a['p_last3']}
...
async def main():
    ...
    await bot.send_message(chat_id=CHAT_ID,
        text="✅ HR SCOUT V5 — FIXED & LIVE\n\n👥 Scans EVERY player on full MLB slate\n🚫 Zero hardcoded players\n\n🎯 Thresholds:\n• Early Look: 62%+\n• Updates: 68%+\n• Final Lock: 75%+\n\n⚡ Scans every 5 minutes\n📈📉 Confidence changes fire instantly\n📝 Every alert explains WHY\n\nRunning first scan now...")

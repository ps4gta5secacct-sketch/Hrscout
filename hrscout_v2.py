...
#  This is step 1 — find the most vulnerable pitchers first
# ============================================================
def pitcher_vulnerability_score(p):
    """
    Score how vulnerable a pitcher is TODAY.
    Higher = more vulnerable = better HR opportunity.
    This drives parlay selection.
    """
    score = 0

    # HR/9 — primary vulnerability indicator
    hr9 = p.get("hr9", 1.2)
    if hr9 >= 2.0:   score += 40
    ...
    elif hr9 < 0.7:  score -= 15  # Elite pitcher — strong penalty

    # Recent trend — getting worse lately
    ...
    elif diff <= -2.0: score -= 10  # Pitcher on a hot streak
...
# ============================================================
#  BATTER QUALITY SCORE
#  Step 2 — find the best hitter in a specific matchup
# ============================================================
def batter_matchup_score(b, p, park, spot, splits):
    """
    Score a batter specifically for THIS matchup.
    Considers platoon, recent form, power metrics.
    """
    score = 0

    # Platoon matchup — specific to this pitcher's splits
    if splits:
        bhand = b.get("hand", "R")
        if bhand == "S":
            p_hr9 = max(splits.get("rhb_hr9", 1.2), splits.get("lhb_hr9", 1.2))
        elif bhand == "R":
            p_hr9 = splits.get("rhb_hr9", p.get("hr9", 1.2))
        else:
            p_hr9 = splits.get("lhb_hr9", p.get("hr9", 1.2))

        if p_hr9 >= 2.5:   score += 30
        elif p_hr9 >= 2.0: score += 24
        elif p_hr9 >= 1.5: score += 18
        elif p_hr9 >= 1.2: score += 12
        elif p_hr9 >= 0.9: score += 5
        elif p_hr9 < 0.6:  score -= 15

        # Massive platoon gap bonus
        gap = abs(splits.get("rhb_hr9", 1.2) - splits.get("lhb_hr9", 1.2))
        if gap >= 1.0 and p_hr9 > p.get("hr9", 1.2):
            score += 8

    # Recent hot streak — most predictive short term factor
    hr7 = b.get("hr7", 0)
    avg7 = b.get("avg7", 0.250)
    if hr7 >= 4:   score += 25
    elif hr7 >= 3: score += 20
    elif hr7 >= 2: score += 14
    elif hr7 >= 1 and avg7 >= 0.300: score += 8
    elif hr7 >= 1: score += 5
    ...
    # Power metrics — Savant data preferred
...
def calculate_confidence(b, p, park, spot=0, splits=None):
    """
    Final confidence score combining pitcher vulnerability + batter matchup.
    Scaled to 25-95% range.
    """
    p_score = pitcher_vulnerability_score(p)
    b_score = batter_matchup_score(b, p, park, spot, splits)

    # Pitcher vulnerability = 45% of total
    # Batter matchup = 55% of total
    raw = (p_score * 0.45) + (b_score * 0.55)

    # Scale to 25-95
    # Raw scores typically range 0-140
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for game in games:
            ...
            if states.get(game_id) == fp:
                # Game unchanged — but still collect top picks for parlay
                # We need to reprocess to build the full parlay pool
                pass
            ...
            # Only send game alert if something changed
            send_game_alert = (states.get(game_id) != fp)
            ...
            # ============================================================
            #  SCORE BATTERS — Pitcher vulnerability first
            # ============================================================
            async def score_lineup(lineup, opp_p, park, is_home):
                ranked = []
                for i, player in enumerate(lineup):
                    ...
                    # Pitcher vulnerability score for this specific matchup
                    p_vuln = pitcher_vulnerability_score(opp_p)
            ...
            away_ranked = await score_lineup(away_lineup, home_p, home_abbrev, False)
            home_ranked = await score_lineup(home_lineup, away_p, home_abbrev, True)

            if not away_ranked and not home_ranked:
                continue

            # Build display lines — top 3 per team
            ...
            if all_cands:
                ...
                # Add interchangeable partner to pool
...
━━━━━━━━━━━━━━━━━━━━━━""".strip()
    ...
    # ============================================================
    #  PARLAY SELECTION — Pitcher vulnerability drives picks
    #  Full slate — all games considered
    #  One player per game — no duplicates
    # ============================================================
    if not parlay_sent and all_top_picks and first_game_time:
        ...
        if 10 <= mins_to_first <= 35:
            ...
            # Pitcher vulnerability (45%) + confidence (35%) + EV value (20%)
            def parlay_sort(p):
                p_vuln = p.get("p_vuln_score", 0)
                conf = p.get("conf", 0)
                ev = p.get("ev_score") or 0
                # EV bonus when players are close in score
                ev_bonus = ev * 0.5 if ev > 0 else 0
                return (p_vuln * 0.45) + (conf * 0.35) + (ev_bonus * 0.20)

            unique.sort(key=parlay_sort, reverse=True)

            # Step 3 — One player per game
            def filter_one_per_game(picks, max_legs):
                used_games = set()
                filtered = []
                for p in picks:
                    game_key = p.get("matchup", "")
                    if game_key not in used_games:
                        used_games.add(game_key)
                        filtered.append(p)
                    if len(filtered) >= max_legs:
                        break
                return filtered

            std_2 = filter_one_per_game(unique, 2)
            std_3 = filter_one_per_game(unique, 3)
            std_4 = filter_one_per_game(unique, 4)

            # EV parlay — sort by pure value, still one per game
            ev_sorted = sorted(unique, key=lambda x: (x.get("ev_score") or 0) + ((x.get("fd_odds") or 200) / 100), reverse=True)
            std_4_names = {p["name"] for p in std_4}
            ev_alt = [p for p in ev_sorted if p["name"] not in std_4_names]
            ev_3 = filter_one_per_game(ev_alt if len(ev_alt) >= 3 else ev_sorted, 3)
            ...
            def leg(p, i):
                ...
                if gt:
                    try:
                        ...
                        pass
                vuln_note = ""
                if p.get("p_hr9", 0) >= 1.5:
                    vuln_note = f" 🚨{p['p_hr9']}HR9"
                return (
                    f"{i}. {p['name']} ({p.get('hand', '')}) — {p['conf']}%{hot}{alt}{vuln_note}\n"
...
async def main():
    ...
    await bot.send_message(chat_id=CHAT_ID,
        text="✅ HR SCOUT V5 — BULLETPROOF BUILD\n\n"
             "🔱 NEW SELECTION LOGIC:\n"
             "1. Find most vulnerable pitchers first\n"
             "2. Find best hitter in that matchup\n"
             "3. Platoon + recent form decides ties\n"
             "4. Higher odds wins when within 3%\n\n"
             "🌅 Full slate parlays — all games considered\n"
             "🔒 One player per game — no duplicates\n"
             "📋 Top 3 per team displayed\n"

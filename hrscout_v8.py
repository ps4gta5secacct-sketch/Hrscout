    ...
    async with aiohttp.ClientSession() as session:
        ...
        for game in games:
            ...
            try:
                ...
                game_dt = dt
                mins_since = (now_utc - dt.replace(tzinfo=datetime.timezone.utc)).total_seconds() / 60
                # Skip only if game already started more than 30 mins ago
                # Future games (negative mins_since) are always included
                if mins_since > 30:
                    continue
            except:
                local_time = "TBD"
            ...
            if not home_lineup and not away_lineup:
                # Use active roster as projected lineup
                # This ensures ALL games are included in parlay pool
...
━━━━━━━━━━━━━━━━━━━━━━""".strip()
    ...
    if not parlay_sent and all_top_picks and first_game_time:
        ...
        # Fire 15-30 mins before FIRST game — but picks from ENTIRE slate
        if 10 <= mins_to_first <= 35:
            ...
            # Always full slate — all games considered regardless of start time
            slate_label = "🌅➡️🌙 FULL SLATE PARLAY"

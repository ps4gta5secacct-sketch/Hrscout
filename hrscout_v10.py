...
def calculate_confidence(b, p, park, spot=0, p_splits=None):
    # Weights designed to reflect TODAY'S SITUATION over reputation
    # Pitcher vulnerability + platoon + recent form = 52% of score
    # Raw power metrics = 37% (still matters but not dominant)
    # Situational = 11%
    weights = {
        # TODAY'S SITUATION — 52%
        "p_hr9":           12,  # Pitcher vulnerability today — Guy 1
        "platoon_splits":  12,  # Real splits vs this batter's hand — deciding factor
        "p_recent_form":   10,  # Pitcher trending worse recently
        "batter_form":     10,  # Batter hot last 7 days
        "p_barrel":         8,  # Barrel % allowed — how hittable today

        # RAW POWER METRICS — 37%
        "barrel":          10,  # Guy 1 + 2
        "ev":               8,  # Guy 1 + 2
        "hard_hit":         5,  # Guy 1
        "flyball":          5,  # Guy 1
        "pull":             5,  # Guy 2
        "iso":              4,  # Guy 1 — reduced to limit reputation bias

        # SITUATIONAL — 11%
        "recent_hr":        5,  # Season HR rate
        "park":             4,  # Park factor today
...
━━━━━━━━━━━━━━━━━━━━━━""".strip()
    ...
    if not parlay_sent and first_game_time and all_top_picks:
        ...
        if mins_to_first <= 35:  # Send parlay anytime within or past window
